package com.automation.page.activities;

import com.automation.page.AbstractPageBase;
...
public class CalendarEventsPage extends AbstractPageBase {
   ...
   //private WebDriverWait wait;
    ...
    private List<WebElement> treeDot;

    @FindBy(css = "[title='Grid Settings']")
    private WebElement gritBtn;

    @FindBy(css = "td[class='title-cell']")
    private List<WebElement> gridSettings;
    ...
    public void checkSaveOptions(){
        for (WebElement each : saveOptions) {
    ...
    public void viewEditDeleteBtn(){
        for (WebElement each : treeDot) {
            Assert.assertTrue(each.isDisplayed());
        }
    }
    ...
    public void clickGridBtn(){
        BrowserUtils.wait(3);
        wait.until(ExpectedConditions.elementToBeClickable(gritBtn)).click();
    }

    public String selectAllGrid(){
        BrowserUtils.wait(2);
        String act="";
        for (int i=0; i<gridSettings.size(); i++){
            if (gridSettings.get(i).getText().contains("Title")){
                continue;
            }
            gridSettings.get(i).click();
            act =  gridSettings.get(0).getText();
        }
        return act;
    }
